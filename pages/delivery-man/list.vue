	...
	<v-app class="pa-5">
		<v-card class="card">
			<v-card-title class="blue-grey lighten-4">
				...
				<v-spacer></v-spacer>
				<v-btn v-permission="'add deliveryman'" class="primary white--text" to="/delivery-man/add">
				...
				<div class="d-flex justify-space-between">
					...
					<div>
						<v-btn class="red darken-1">
							<a :href="baseURL + '/api/deliveryman/export-pdf'" class="nuxt--link">PDF</a>
						</v-btn>
						<v-btn class="lime lighten-1">
							<a :href="baseURL + '/api/deliveryman/export-csv'" class="nuxt--link">CSV</a>
						</v-btn>
					...
					<template v-slot:item="{ item }">
						<tr class="sale-tr">
							...
							<td>
								...
								<v-btn
									v-permission="'delete deliveryman'"
...
<script>
	export default {
		...
		methods: {
			...
			searchItems() {
				this.$axios
					.$get(`/api/delivery-man?search=${this.search}`)
					.then(res => {
						this.items = res.deliveryman.data;
						// console.log(res);
					})
			...
			editItem(id) {
				this.$router.push(`/delivery-man/${id}/edit`);
			},

			// viewInfo(id) {
			// 	this.$router.push(`/deliveryman/${id}/show`);
			// }
