			...
			<v-row>
				<v-col md="6" cols="12">
					<div class="d-flex flex-column px-10 pt-5">
						...
						<span class="font-weight-medium">
							References:
							<span class="font-weight-regular">{{ sale.reference_no }}</span>
						</span>
						<span class="font-weight-medium" v-if="sale.branch">
							Warehouse:
							<span class="font-weight-regular">{{ sale.branch.name }}</span>
						</span>
						<span class="font-weight-medium">
							Payment Status:
							<span class="font-weight-regular">{{sale.payment_status }}</span>
						</span>
					</div>
				</v-col>
				<v-col md="6" cols="12">
					...
					<table class="tableSale">
						...
						<template v-if="sale.products && sale.products.length > 0">
							<tr v-for="i in sale.products">
								...
								<td
									class="tableSale--tr"
								>USD {{ (i.pivot.unit_price - (i.pivot.unit_price * i.pivot.discount / 100)) * i.pivot.quantity | formatMoney }}</td>
							</tr>
						</template>
						<template v-else>
							<tr>
